...
import { Globe, ArrowRight, User, Briefcase, ArrowLeft, Home } from "lucide-react";
...
export default function Login() {
  ...
  const onSubmit = async (data: LoginForm) => {
    console.log('[Login] Attempting sign in with:', { email: data.email, role: data.role });
    setIsLoading(true);
    ...
    try {
      ...
      // If user has a role that doesn't match the selected role
      if (userRole && userRole !== data.role) {
        throw new Error(`Please sign in as ${userRole} instead.`);
      }

      // Set user role if it doesn't exist
      if (!userRole) {
      ...
      console.error('[Login] Error during sign in:', {
        code: error.code,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary via-accent to-primary/80 flex items-center justify-center p-4 relative overflow-hidden">
      {/* Background decoration - subtle world map pattern */}
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-11 text-base bg-gradient-to-r from-primary to-accent hover:opacity-90 transition-opacity"
            >
          ...
          <div className="space-y-2 text-center text-sm">
            <p className="text-muted-foreground">
              ...
              <Link to="/youth-signup" className="text-primary hover:underline font-medium">
      ...
      <div className="absolute top-4 left-4 md:top-6 md:left-6 z-50">
        <Link
          to="/"
          className="group flex items-center gap-2 px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary transition-colors bg-white/80 backdrop-blur-sm rounded-md border border-border hover:border-primary/20 shadow-sm"
        >
